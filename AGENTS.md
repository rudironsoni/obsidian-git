...
hand-authored Cursor Cloud config. They are not Rulesync output.
...
pnpm run lint         # ESLint for src, tests, scripts, and vitest.config.ts
...
pnpm run rulesync:generate  # CI-owned; do not run to hand-commit outputs
pnpm run rulesync:check     # fail if generated agent files are stale
```
...
-   `pnpm run test` runs Vitest against plugin modules with the Obsidian stub
    and temporary Git repositories. This is the default gate and the only
    layer available in Cursor Cloud.
-   `pnpm run all` is the local quality gate: tsc, Svelte check, Prettier,
    ESLint, and Vitest. It does not launch Obsidian.
-   `pnpm run build` produces the ignored root `main.js` release artifact.
-   `pnpm run test:e2e` runs WDIO + `wdio-obsidian-service` against a
    sandboxed Obsidian with this plugin loaded. GitHub Actions uses xvfb.
    Do not claim UI or WasmGit-in-Obsidian bugs are fixed from Vitest alone.
-   Optional local loop: official Obsidian CLI (`obsidian-cli` on macOS) and
    `@wdio/mcp` for a live WDIO session. Not a CI gate.
