...
- **Apply UI to the main window via `this.app.workspace.containerEl.ownerDocument`, not `activeDocument`**, for anything that must affect the editing window from a settings change: `<body>` classes, CSS variables, `<style>` injection, ribbon/toolbar button swaps. Since Obsidian 1.13 opens Settings in a separate window, `activeDocument`/`activeWindow` point at the Settings window during an `onChange`, so the change lands in the wrong window and only takes effect after restart. See [code-patterns.md](code-patterns.md#multi-window-target-the-app-window-not-activedocument).
