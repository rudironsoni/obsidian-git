...
name: plugin-test
description: >-
    Acceptance test workflow for the obsidian-git plugin. Use when the user
    asks to test the plugin, smoke a change, or judge whether WasmGit, UI, or
    editor behavior still works. Layers: Vitest, WDIO in Obsidian, optional
    local CLI. Does not spend API tokens and does not commit source changes.
targets:
    - "*"
opencode:
    metadata:
